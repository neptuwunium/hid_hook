...
namespace {
	...
	HINSTANCE hook_library = nullptr;
} // namespace
...
DllMain(HMODULE, const DWORD reason, LPVOID) -> BOOL {
	if (reason == DLL_PROCESS_ATTACH) {
		SetErrorMode(0);
		hook_library = LoadLibraryA(HOOK_NAME ".dll");

		if(hook_library == nullptr) {
			const auto err = GetLastError();
			std::ofstream log;
			log.open("./hid.log");
			log << "cannot load " HOOK_NAME  ".dll: " << std::hex << std::setfill('0') << std::setw(8) << "0x" << err;
